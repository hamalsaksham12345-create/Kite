...
<head>
    ...
    <title>Edit {{ $category->name }} - {{ $currentRestaurant->name ?? 'Restaurant' }} Admin</title>
    ...
    <div class="min-h-screen">
            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="flex justify-between h-16">
                    <div class="flex items-center space-x-8">
                        ...
                        <h1 class="text-2xl font-black text-black">Edit {{ $category->name }}</h1>
                    </div>
                </div>
            </div>
        </nav>
        ...
        <div class="max-w-2xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
            <div class="bg-white border-2 border-black hover:shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] transition-all duration-200">
                <div class="p-8">
                    <form action="{{ route('admin.categories.update', ['restaurant_slug' => $currentRestaurant->slug, 'category' => $category]) }}" 
                          method="POST" 
                        ...
                        <div>
                            <label for="image" class="block text-lg font-black text-black mb-3">
                                {{ $category->image ? 'Replace Image (Optional)' : 'Category Image (Optional)' }}
                            </label>
                            <input id="image" 
                                   name="image" 
                                   type="file" 
