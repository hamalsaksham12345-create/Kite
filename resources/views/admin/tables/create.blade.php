...
<head>
    ...
    <title>Create Table - {{ $currentRestaurant->name ?? 'Restaurant' }} Admin</title>
    ...
    <div class="min-h-screen">
            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="flex justify-between h-16">
                    <div class="flex items-center space-x-8">
                        ...
                        <h1 class="text-2xl font-black text-black">Create New Table</h1>
                    </div>
                </div>
            </div>
        </nav>
        ...
        <div class="max-w-2xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
            <div class="bg-white border-4 border-black hover:shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] transition-all duration-200">
                <div class="p-8">
                    <form action="{{ route('admin.path.tables.store', $currentRestaurant->slug) }}" 
                          method="POST" 
                          class="space-y-6">
                        @csrf
                        ...
                        <div>
                            ...
                            <textarea id="notes" 
                                      ...
                                      placeholder="e.g., Near window, Corner table, etc.">{{ old('notes') }}</textarea>
                            @error('notes')
                                <p class="mt-2 text-sm font-bold text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
